...
export default function PresupuestosPage({ hideHeader = false }: { hideHeader?: boolean }) {
  const { token } = useAuth();
    ...
    <div className="space-y-10 animate-in fade-in slide-in-from-bottom-4 duration-700 pb-10">
      ...
      >
        <form className="grid grid-cols-1 gap-4 pt-4" onSubmit={e => {
          ...
          createMutation.mutate({ ordenId: Number(fd.get('ordenId')) });
        }}>
          <div className="flex flex-col gap-1">
            <label className="text-xs font-bold text-slate-500 uppercase">Orden de Trabajo *</label>
            <Dropdown name="ordenId" options={ordenOptions} placeholder="Seleccionar orden…" required className="rounded-xl" />
          </div>
        ...
        {editItem && (
          <form className="grid grid-cols-1 gap-4 pt-4" onSubmit={e => {
            ...
            updateMutation.mutate({ 
              id: editItem.id, 
              data: { estado: String(fd.get('estado')) } 
            });
          }}>
            <div className="flex flex-col gap-1">
              <label className="text-xs font-bold text-slate-500 uppercase">Estado del Presupuesto</label>
              <Dropdown name="estado" value={String(editItem.estado).toUpperCase()} options={STATUS_OPTS} className="rounded-xl" />
            </div>
            <div className="flex justify-end gap-2 mt-4">
